...
import uuid
import shutil
from . import remboursement_data  # Import relatif
from . import remboursement_workflow  # Import relatif
from config.settings import REMBOURSEMENT_BASE_DIR, STATUT_CREEE

# Réexporter les fonctions principales nécessaires aux contrôleurs
obtenir_toutes_les_demandes = remboursement_data.charger_toutes_les_demandes_data  # Alias
supprimer_demande_par_id = remboursement_data.supprimer_demande_par_id_data  # Alias
...
def creer_nouvelle_demande(
    ...
    """Prépare les données et la structure de la nouvelle demande, puis l'enregistre."""
    ...
    try:
        ...
        return None  # Ne pas continuer si le dossier ne peut être créé
    ...
    if chemin_facture_source:
        ...
        try:
            ...
            # La création continue même si la copie de la facture échoue car elle est optionnelle

    base_nom_rib = os.path.basename(chemin_rib_source)
    nom_fichier_rib = f"RIB_{remboursement_data._sanitize_directory_name(nom)}_{remboursement_data._sanitize_directory_name(prenom)}_{base_nom_rib}"
    ...
    try:
        ...
        # Si le RIB (obligatoire) ne peut être copié, la demande ne doit pas être créée
...
# Les anciennes fonctions de verrouillage sont supprimées d'ici.