...
from config.settings import SMTP_CONFIG  # Importer la configuration
...
def envoyer_email_reset(destinataire_email: str, nom_utilisateur: str, code_reset: str) -> bool:
    ...
    try:
        ...
        elif SMTP_CONFIG.get('use_tls', True):  # TLS par défaut si SSL n'est pas explicitement true
            server = smtplib.SMTP(SMTP_CONFIG['server'], SMTP_CONFIG['port'])
            server.starttls(context=context)
        else:  # Connexion non sécurisée (non recommandée)
            server = smtplib.SMTP(SMTP_CONFIG['server'], SMTP_CONFIG['port'])
