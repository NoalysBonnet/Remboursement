...
class CreationDemandeDialog(ctk.CTkToplevel):
    def __init__(self, master, remboursement_controller):
    ...
    def _selectionner_facture(self):
        ...
        if chemin.lower().endswith(".pdf"):
            infos = self.remboursement_controller.extraire_info_facture_pdf(chemin)

            # --- SECTION CORRIGÉE ---
            # Au lieu d'une boucle générique, on assigne chaque champ explicitement.
            # Cela garantit que la "reference" du PDF va dans le champ "reference_facture".
            if infos.get("nom"):
