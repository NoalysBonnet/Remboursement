...
import datetime
import json  # Non utilisé directement ici, mais gardé au cas où
...
from views.document_history_viewer import DocumentHistoryViewer  # S'assurer que cet import est présent
...
class MainView(ctk.CTkFrame):
    ...
    def _action_voir_historique_docs(self, demande_data: dict):  # DÉFINITION AJOUTÉE ICI
        callbacks_pour_historique = {
            'voir_pj': self._action_voir_pj,
            'dl_pj': self._action_telecharger_pj
        }
        DocumentHistoryViewer(self, demande_data, callbacks_pour_historique)
    ...
    def _action_mlupo_accepter(self, id_demande: str):
        ...
        def _submit():
            ...
            succes, msg = self.remboursement_controller.mlupo_accepter_constat(id_demande, current_pj_path, commentaire)
            if succes:
    ...
    def _ouvrir_fenetre_creation_demande(self):
        ...
        btn_soumettre = ctk.CTkButton(form_frame, text="Enregistrer la Demande", command=soumettre_demande, height=35)
