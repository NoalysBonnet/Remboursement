...
class DocumentViewerWindow(ctk.CTkToplevel):
    def __init__(self, master, file_path: str, title: str):
        ...
        # Ce conteneur sera utilisé pour empiler les pages PDF ou pour afficher une seule image/texte
    ...
    def load_and_display_document(self):  #
        for widget in self.content_container.winfo_children():  # Nettoyer le conteneur de contenu
            widget.destroy()
        ...
        try:  #
            ...
            if ext in ("png", "jpg", "jpeg", "gif", "bmp"):  #
                ...
                if self.pdf_doc.page_count == 0:  #
                    ...
                    # La hauteur initiale peut être basée sur la première page ou une hauteur max raisonnable
            ...
            else:  # Type non supporté explicitement pour affichage direct
                error_message_detail = f"Aperçu direct non supporté pour '{os.path.basename(self.file_path)}'."  #

            if not file_processed_internally:  #
                # Afficher le message d'erreur/info et proposer l'ouverture externe
        ...
        except Exception as e:  # Erreur générale de chargement/rendu
            print(f"Erreur détaillée DocumentViewer: {e}")  #
            for widget in self.content_container.winfo_children(): widget.destroy()  # Nettoyer
